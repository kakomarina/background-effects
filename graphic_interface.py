...
def background_effects():
    ...
    canvas = tkr.Canvas(root2, width=largura, height=altura) #defines dimensions to interface
    canvas.grid()
...
def image_interface():
    ...
    label = tkr.Label(root, text="Generating clustered image...", font=large_font)
    ...
    imageio.imwrite("clustered_img.jpg", img_clustered.astype(np.uint8)) #saves file with clustered image
    
    destroy_root(root) #destroys interface page 0
    
    root1 = tkr.Tk() #master of interface page 1

    canvas = tkr.Canvas(root1, width=largura, height=altura) #defines interface dimensions
    canvas.grid()

    clust_img = ImageTk.PhotoImage(Image.open("clustered_img.jpg")) #opens clustered image
    canvas.create_image(0, 0, anchor="nw", image=clust_img) #sets image in interface
    
    #root1.bind("<Motion>", motion) #gets mouse location in the interface
    root1.bind("<Button-1>", getColor) #if the mouse is clicked, is called a function (getColor) to get the clicked pixel color   
